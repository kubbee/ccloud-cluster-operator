...
func (r *CCloudSchemaRegistryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	cccloudSchemaRegistry := &messagesv1alpha1.CCloudSchemaRegistry{}

	if err := r.Get(ctx, req.NamespacedName, cccloudSchemaRegistry); err != nil {
		return reconcile.Result{}, client.IgnoreNotFound(err)
	}

	if !cccloudSchemaRegistry.ObjectMeta.DeletionTimestamp.IsZero() {
		logger.Info("Deleting")
		return reconcile.Result{}, nil // implementing the nil in the future
	}

	if cccloudSchemaRegistry.Spec.Foo != "" {
		fmt.Println("hello")
	}

	return r.declareSchemaRegistry(ctx, req)
}

//
func (r *CCloudSchemaRegistryReconciler) declareSchemaRegistry(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := ctrl.LoggerFrom(ctx)
